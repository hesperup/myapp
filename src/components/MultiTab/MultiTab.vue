<template>
  <a-tabs v-if="multTab.length" v-model:activeKey="activeKey" type="editable-card" hide-add @edit="onClone"
    @change="onChangeTab">
    <a-tab-pane v-for="tab in multTab" :closable="multTab.length != 1" :key="tab.path" :tab="$t(tab.title)"></a-tab-pane>
  </a-tabs>
</template>
<script setup>
